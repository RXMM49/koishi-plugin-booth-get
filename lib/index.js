...
var Config = import_koishi.Schema.object({
  ...
  // 卡片样式配置
  cardWidth: import_koishi.Schema.natural().min(400).max(1200).description("卡片宽度（像素）").default(900),
  leftColumnWidth: import_koishi.Schema.natural().min(200).max(500).description("左侧栏宽度（像素）").default(280),
  
  // 颜色主题配置
  topGradientColor1: import_koishi.Schema.string().description("顶部渐变起始颜色").default('#e8bdf6'),
  topGradientColor2: import_koishi.Schema.string().description("顶部渐变结束颜色").default('#fceabb'),
  footerGradientColor1: import_koishi.Schema.string().description("底部渐变起始颜色").default('#fceabb'),
  footerGradientColor2: import_koishi.Schema.string().description("底部渐变结束颜色").default('#e8bdf6'),
  priceTagBgColor: import_koishi.Schema.string().description("价格标签背景颜色").default('#ff66b2'),
  infoBubbleBgColor: import_koishi.Schema.string().description("说明气泡框背景颜色").default('#aafcf5'),
  textColor: import_koishi.Schema.string().description("主要文字颜色").default('#d63384'),
  shopBannerBgColor: import_koishi.Schema.string().description("店铺横幅背景颜色").default('#fcfcd0')
